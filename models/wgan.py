...
from models import get_gradient, get_noise, gradient_penalty
from models import create_node_model, create_fcn_model
...
class WGAN:
    ...
    def create_generator(self):
        if self.params["generator_architecture"].lower() == "node":
            ...
            self.generator_model = create_node_model(
            ...
            logger.info(f"params: \n{self.params['generator']}")
            self.generator_model = create_fcn_model(
                self.params["generator"]["params"],
                model_type=self.params["generator_model_type"],
            )
        else:
        ...
        if generator_checkpoint_path:
            ...
            self.generator_model.load_state_dict(
                generator_checkpoint["generator_model_state_dict"]
            )
    ...
    def create_critic(self):
        if self.params["critic_architecture"].lower() == "node":
            logger.info("Creating NODE model")
            logger.info(f"params: \n{self.params['critic']}")
            self.critic_model = create_node_model(
                self.params["critic"]["params"],
                model_type=self.params["critic_model_type"],
            )
        elif self.params["critic_architecture"].lower() == "fcn":
            logger.info("Creating FCN model")
            logger.info(f"params: \n{self.params['critic']}")
            self.critic_model = create_fcn_model(
                self.params["critic"]["params"],
                model_type=self.params["critic_model_type"],
            )
        else:
        ...
        if critic_checkpoint_path:
            ...
            self.critic_model.load_state_dict(
                critic_checkpoint["critic_model_state_dict"]
            )
    ...
    def train_critic(self, batch):
        ...
        epsilon = torch.rand(
            real_full.size(0), 1, 1, 1, device=device, requires_grad=True
        )
        gradient = get_gradient(
            self.critic_model, real_full, generated.detach(), epsilon=epsilon
        )
        gp = gradient_penalty(gradient)
        crit_loss = (
            torch.mean(crit_fake_pred - crit_real_pred) + self.params["c_lambda"] * gp
        )
    ...
    def generate(self, batch):
        ...
        noize = get_noise(x.shape[0], self.params["noise_dim"]).to(
            self.params["device"]
        )
        noized_x = torch.cat(
            [x, noize],
    ...
    def validation_step(self, generated, real, weights=None):
        ...
        outputs = {"histogram": figure, "rocauc": roc_auc_score}
        return outputs
